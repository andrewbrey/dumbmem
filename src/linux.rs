...
impl Proc {
    ...
    pub fn smaps(&self) -> String {
        match self {
            Proc::This => "/proc/self/smaps".to_string(),
            Proc::Other(id) => format!("/proc/{id}/smaps"),
        }
    }

    pub fn statm(&self) -> String {
        match self {
            Proc::This => "/proc/self/statm".to_string(),
            Proc::Other(id) => format!("/proc/{id}/statm"),
        }
    }
}

pub fn memory_stats(which: Proc) -> Option<MemoryStats> {
    ...
    #[cfg(not(feature = "always_use_statm"))]
    if let Ok(false) =
        SMAPS_CHECKED.compare_exchange(false, true, Ordering::Relaxed, Ordering::Relaxed)
    {
        let smaps_exist = fs::metadata(which.smaps()).is_ok();
    ...
    if SMAPS_EXIST.load(Ordering::Relaxed) {
        return match fs::read_to_string(which.smaps()) {
            Ok(smap_info) => {
    ...
    match fs::read_to_string(which.statm()) {
        Ok(statm_info) => {
